...
public class AlumnosRepositoryImpl implements AlumnosRepository{
	...
	public Integer insertAlumno(AlumnosDto alumno) {
		return jdbcTemplate.update("INSERT INTO ALUMNO VALUES (SEQ_ALUMNO.NEXTVAL,?,?)", 
				new Object[] {alumno.getNombre(), alumno.getIdEscuela()}  );
	}

	@Override
	public Integer updateAlumno(AlumnosDto alumno) {
		return jdbcTemplate.update("UPDATE ALUMNO SET NOMBRE = ?, ESCUELA = ? WHERE ID_ALUMNO = ?", 
				new Object[] {alumno.getNombre(), alumno.getIdEscuela(), alumno.getIdAlumno()}  );
	}

	@Override
	public Integer deleteAlumno(AlumnosDto alumno) {
		return jdbcTemplate.update("DELETE FROM ALUMNO WHERE ID_ALUMNO = ?", 
				new Object[] {alumno.getIdAlumno()}  );
	}
